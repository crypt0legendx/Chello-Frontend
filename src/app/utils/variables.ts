import { Injectable } from '@angular/core';

@Injectable()
export class veriables{

    chelloCommission: any = 6;
    creatorKeeps: any = 94;

    referralCommission = 2;

    adminMonthlyFee: any = 25;
    adminYearlyFee: any = 250;

    supportEmail: any = "support@chello.biz";
    complaintsEmail: any = "complaints@chello.biz";
    privacyEmail: any = "privacy@chello.biz";
    paymentEmail: any = "payment@chello.biz";
    mediaEmail: any = "media@chello.biz";

    facebookId: any = "javascript:void(0)";
    instagramId: any = "https://www.instagram.com/letschello/";
    tiktokId: any = "https://tiktok.com/@chello.world";
    twitterId: any = "https://twitter.com/LetsChello";
}