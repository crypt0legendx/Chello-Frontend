...
declare var $: any;
  ...
  @Component({
    selector: 'app-timed-post',
    templateUrl: '../pages/timed-post.component.html',
    styleUrls: ['../pages/timed-post.component.scss'],
    providers: [PostListComponent, GroupPostListComponent]
  })

  export class TimedPostComponent {
  ...
  // arakidev branch for timed post
  public timeOptionSelected: boolean = false;
  public selectedTimeOption : string = '';
  ...
  /**
   * araki dev function
   * @param opt_num  post option number
   */
  ...
  /**
   * 
   * @param type  time post type
   */
  setTimedPost(type: boolean) {
    if(type) {   // disappearing selected
      
      this.timeOptionSelected = true;
      this.selectedTimeOption = 'disappearing';

    } else {     // comming selected
